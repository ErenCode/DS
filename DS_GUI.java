	...
	public DS_GUI() {
	    ...
	    JMenuNew.addMouseListener(new MouseAdapter(){  
			public void mouseClicked(MouseEvent e) {
				...
				/*
				JFrame newgui=new DS_GUI();
				newgui.setBounds(100, 100, 600, 437);
				newgui.setTitle("Shared Whiteboard");
	    ...
	    JMenuSave.addMouseListener(new MouseAdapter(){  
			public void mouseClicked(MouseEvent e) {
				...
				try {
					/*
					int height=shape.getBufImage().getHeight();
					int width=shape.getBufImage().getWidth();
					int x=shape.getX();
					int y=shape.getY();
					myImage = new Robot().createScreenCapture(new Rectangle(x,y,width,height));
					*/
					myImage=shape.getBufImage();
					ImageIO.write(myImage, "jpg", new File("./image.jpg"));
				} catch (IOException e1) {
					// TODO Auto-generated catch block
					e1.printStackTrace();
				}	
